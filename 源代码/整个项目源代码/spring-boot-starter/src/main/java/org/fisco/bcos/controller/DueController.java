...
public class DueController {
    ...
    @GetMapping("/due")
    public String due(Model model,
                      HttpSession session) throws Exception {
        String username = session.getAttribute("loginUser").toString();
        System.out.println("正在due页面的是:"+username);
        int n = m.getMyDueNum(username).send().intValue();
        System.out.println("应付账款记录数量:"+n);
        List<Receipt> allReceipt = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            BigInteger index = m.getMyDueIndex(username,new BigInteger(i+"")).send();
            //BigInteger index = new BigInteger(_index+"");
