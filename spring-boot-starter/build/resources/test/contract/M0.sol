...
contract M0{
    ...
    */
    ...
    /*6.展示应还账款——solidity没办法返回结构体和变成数组,
        （1）拿到手里的账单数量
        （2）逐个判断账单是否还清,0还清，没有还清的返回账单号+1,因为uint不能返回负数，暂且这样处理
            ——也就是说真正拿到的账单号会大1，但是不影响账单数量的遍历
        （3）查询有效的账单
    */
    function getMyDueNum(string payer) constant public returns (uint) {
        return myDue[payer].length;
    }
    function isMyDueValid(string payer, uint i) constant public returns(uint) {
        if(receiptMap[myDue[payer][i]].finish==false)return myDue[payer][i]+1;
        return 0;
    }
    function checkReceiptByNum(uint n) constant public returns(string payer, string payee, uint amount, uint year,uint month, uint day){
        uint no = n-1;
        return (receiptMap[no].payer,receiptMap[no].payee,receiptMap[no].amount,receiptMap[no].year,receiptMap[no].month,receiptMap[no].day);
    }
    /*7.展示应收账款
        （1）拿到手里的账单数量
        （2）逐个判断账单是否还清,-1已收到，没有收到的返回账单号
        （3）查询有效的账单
    */
    function getMyReceiNum(string payee) constant public returns (uint){
        return myRecei[payee].length;
    }
    function isMyReceiValid(string payee, uint i) constant public returns (uint) {
        if(receiptMap[myRecei[payee][i]].finish==false)return myRecei[payee][i]+1;
        return 0;
    }
}
/*合约的展示功能：
暂且改成，所有账单都会展示。
记得前面交易要展示交易是否能成立。——公司是否存在。
*/